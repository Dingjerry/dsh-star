...
function matchesCatalogQuery(item, query) {
    ...
    if (categories.length > 0 && item.categories?.some(value => categories.includes(value)) !== true)
        return false;
    const capabilities = new Set([
        ...(item.capabilities?.required ?? []),
        ...(item.capabilities?.optional ?? []),
    ]);
    if ((query.capability ?? []).some(value => !capabilities.has(value)))
        return false;
...
function sortCatalogItems(items, query) {
    if (query.sort === undefined || query.sort === 'relevance' || query.sort === 'downloads')
        return items;
...
function validateCompleteCatalogScan(source, values) {
    if (values.length > MAX_CATALOG_PAGES)
    ...
    for (const value of values) {
        ...
        if (snapshot.source.providerRevision !== undefined)
            revisions.add(snapshot.source.providerRevision);
        if (revisions.size > 1)
            throw new Error('catalog scan changed provider revision');
        if (snapshot.page.total !== undefined) {
            ...
            if (expectedTotal > MAX_CATALOG_ITEMS)
                throw new Error('catalog scan exceeded the item limit');
        }
        for (const item of snapshot.items) {
            if (item.provenance.sourceRecordId !== source.sourceRecordId
            ...
            if (itemIds.has(item.id))
                throw new Error('catalog scan contained duplicate item IDs');
            itemIds.add(item.id);
            itemCount += 1;
...
export class DefaultCatalogService {
    store;
    http;
    catalogScanCache = new Map();
    cursors = new Map();
    sourceGenerations = new Map();
    catalogScanGenerations = new Map();
    catalogScanControllers = new Map();
    catalogScanGates = new Map();
    cursorTtlMs;
    maxCursorEntries;
    ...
    invalidateSource(sourceRecordId) {
        ...
        for (const [key, entry] of this.catalogScanCache) {
            if (entry.sourceRecordId === sourceRecordId)
    ...
    purgeExpiredCursors() {
        ...
        for (const [token, entry] of this.cursors) {
            if (now - entry.savedAt >= this.cursorTtlMs)
    ...
    revokeSourceCursors(sourceRecordId) {
        for (const [token, entry] of this.cursors) {
            if (entry.cursor.sourceRecordId === sourceRecordId)
                this.cursors.delete(token);
        }
    }
    ...
    async scanCatalog(signal, options = {}) {
        ...
        if (options.expectedSourceRecordId !== undefined
        ...
        if (cached !== undefined) {
            if (this.now() >= cached.expiresAt)
                this.revokeSourceCursors(source.sourceRecordId);
            this.catalogScanCache.delete(key);
        }
        return await scanGate.run(signal, async () => await this.sourceConcurrency.run(signal, async () => {
            ...
            if (cachedAfterWait !== undefined) {
                if (this.now() >= cachedAfterWait.expiresAt)
                    this.revokeSourceCursors(source.sourceRecordId);
                this.catalogScanCache.delete(key);
            }
            const adapter = adapters.get(source.adapterId);
            if (adapter === undefined)
                throw new Error('catalog adapter unavailable');
            const invalidationController = new AbortController();
            const controllers = this.catalogScanControllers.get(key) ?? new Set();
            ...
            const http = {
                getJson: async (url, requestSignal, policy = {}) => await delegate.getJson(url, requestSignal, options.force === true ? { ...policy, cacheMode: 'reload' } : policy),
            };
            ...
            try {
                ...
                if (adapter.scanCatalog !== undefined) {
                    ...
                    while (true) {
                        ...
                        if (cursors.has(cursor))
                            throw new Error('catalog scan cursor repeated');
                        cursors.add(cursor);
                ...
                for (const snapshot of entry.snapshots) {
                    ...
                    catch { /* installation is optional; catalog browsing remains available */ }
                }
    ...
    queryCatalog(index, value, scope) {
        if (scope !== undefined && (typeof scope.sourceRecordId !== 'string'
            || scope.sourceRecordId.length === 0
            || scope.cursor !== undefined && (typeof scope.cursor !== 'string'
        ...
        if (baseQuery.locale !== index.locale)
            throw new Error('catalog index locale does not match the query');
        const query = scope?.cursor === undefined
            ? baseQuery
            : this.applyCursor(scope.cursor, scope.sourceRecordId, baseQuery, index.sourceGeneration);
        const filtered = sortCatalogItems(completeItems(index).filter(item => matchesCatalogQuery(item, query)), query);
        ...
        const limit = Math.min(query.limit ?? 50, index.source.adapterId === DSH_1024STORE_ADAPTER_ID ? 50 : 100);
        const end = Math.min(offset + limit, filtered.length);
        const baseSnapshot = index.snapshots[0];
        if (baseSnapshot === undefined)
            return [{ source: index.source, stale: false }];
