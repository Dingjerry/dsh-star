...
function requireOrigin(value, expectedOrigin, label) {
    ...
    if (url.origin !== expectedOrigin)
        throw new Error(`${label} changed the registered source origin`);
    return url;
}
export function assertStandardSourceTrustRoot(manifestUrlValue, manifestFinalUrl, endpointValue) {
...
export const standardHttpAdapter = {
    adapterId: 'market.standard-http-v1',
    async fetch(queryValue, context) {
        if (context.source.manifestUrl === undefined)
            throw new Error('standard source has no manifest URL');
        const registeredOrigin = safeHttpsUrl(context.source.manifestUrl, 'standard source manifest URL').origin;
        const manifestResponse = await context.http.getJson(context.source.manifestUrl, context.signal, { allowedOrigin: registeredOrigin });
        ...
        const sourceOrigin = assertStandardSourceTrustRoot(context.source.manifestUrl, manifestResponse.finalUrl, manifest.transport.endpoint);
