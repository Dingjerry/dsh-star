...
function plainText(value, maxLength, allowEmpty = false) {
...
function parseRawPage(value, expectedPage) {
    ...
    if (raw === undefined || !Array.isArray(raw.data))
        throw new Error('dshfind response is invalid');
    ...
    if (page !== expectedPage)
        throw new Error('dshfind response page did not match the request');
    if (perPage !== DSHFIND_PAGE_SIZE)
        throw new Error('dshfind response changed the requested page size');
    if (total > MAX_DSHFIND_ITEMS)
        throw new Error('dshfind catalog exceeded the item limit');
    if (totalPages > MAX_DSHFIND_PAGES)
        throw new Error('dshfind catalog exceeded the page limit');
    const calculatedPages = total === 0 ? 0 : Math.ceil(total / DSHFIND_PAGE_SIZE);
    if (totalPages !== calculatedPages)
        throw new Error('dshfind response page metadata is inconsistent');
    if (totalPages > 0 && page > totalPages)
        throw new Error('dshfind response page exceeded total_pages');
...
function repositoryFromItem(raw) {
    if (typeof raw.repository_url !== 'string')
        return undefined;
    try {
...
function keywords(value, languageValue) {
    ...
    for (const raw of tags) {
        ...
        if (keyword === undefined || seen.has(keyword))
            continue;
        seen.add(keyword);
        result.push(keyword);
        if (result.length === maximumTags)
            break;
    }
 ...
 */
function reviewedNpmTarget(install) {
    if (install === undefined || !Array.isArray(install.methods))
        return undefined;
...
function normalizeItem(value, context) {
    ...
    if (raw.is_risky === true)
        return undefined;
    const id = plainText(raw.full_name, 160);
    const name = plainText(raw.name, 120);
    if (id === undefined || name === undefined || !IDENTIFIER_PATTERN.test(id))
        return undefined;
...
function querySnapshot(query, snapshots) {
    ...
    if (first === undefined)
        throw new Error('dshfind scan did not produce a snapshot');
    ...
    let items = snapshots.flatMap(snapshot => snapshot.items).filter(item => {
        if (hasUnsupportedCapabilities)
            return false;
        if (categories.size > 0 && item.categories?.some(category => categories.has(category)) !== true)
            return false;
        if (search === undefined)
            return true;
        return [
    ...
    if (query.sort === 'name') {
        items = [...items].sort((left, right) => left.displayName.localeCompare(right.displayName, query.locale ?? 'en', { sensitivity: 'base' }));
    }
...
export function createDshfindAdapter(options = {}) {
    ...
    const scanCatalog = async (_query, context) => {
        ...
        while (true) {
            ...
            const response = await context.http.getJson(pageUrl(expectedPage, dataset?.dataVersion), context.signal, { allowedOrigin: DSHFIND_ORIGIN });
            context.signal.throwIfAborted();
            ...
            else if (page.total !== dataset.total
            ...
            for (const rawItem of page.data) {
                ...
                if (rawId !== undefined && IDENTIFIER_PATTERN.test(rawId)) {
                    ...
                    if (seen.has(duplicateKey))
                        throw new Error('dshfind catalog contains duplicate item IDs');
            ...
            if (page.totalPages === 0 || expectedPage >= page.totalPages)
                break;
            if (expectedPage >= MAX_DSHFIND_PAGES)
                throw new Error('dshfind catalog exceeded the page limit');
            await waitForNextPage(interPageDelayMs, context.signal);
            expectedPage += 1;
        }
        if (dataset === undefined)
            throw new Error('dshfind scan did not return dataset metadata');
