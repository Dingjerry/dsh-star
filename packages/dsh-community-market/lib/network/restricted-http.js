...
export function pinnedLookupResult(options, pinned) {
...
async function fetchJson(start, signal, resolveAddress, request, allowedOrigin, redirectCount = 0) {
    ...
    if (status >= 300 && status < 400) {
        ...
        if (location === undefined)
            throw new CatalogNetworkError('redirect');
        return await fetchJson(new URL(location, url).href, signal, resolveAddress, request, allowedOrigin, redirectCount + 1);
    }
...
export function createRestrictedHttpClient(options = {}) {
    const syntheticProxyHostnames = new Set((options.syntheticProxyHostnames ?? []).map(hostname => hostname.toLowerCase()));
...
function awaitCachedResponse(promise, signal, release) {
    ...
    return new Promise((resolve, reject) => {
        ...
        signal.addEventListener('abort', onAbort, { once: true });
        void promise.then(response => finish(() => resolve(response)), cause => finish(() => reject(cause)));
    });
}
/** Cache completed catalog responses and collapse concurrent reads by URL. */
export function createCachedCatalogHttpClient(delegate, options = {}) {
        ...
        async getJson(url, signal, policy = {}) {
            ...
            if (entry.inFlight === undefined) {
                ...
                void request.then(response => {
                    if (entry.inFlight !== request || inFlightController.signal.aborted)
                        return;
...
export const restrictedHttpClient = createRestrictedHttpClient();
