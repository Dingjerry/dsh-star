...
function candidateKey(sourceRecordId, itemId) {
...
function assertRuntimeCompatibility(manifest) {
    ...
    for (const field of ['dependencies', 'peerDependencies', 'optionalDependencies']) {
        ...
        for (const [name, range] of Object.entries(dependencies)) {
            if (name === 'cordis') {
                throw new MarketInstallError('verification-failed', 'The plugin package depends on the legacy Cordis runtime and is not compatible with DSH Star.');
            }
            ...
            if (!accepts(runtimeVersion, range)) {
                throw new MarketInstallError('verification-failed', 'The plugin package is not compatible with this DSH Star runtime.');
            }
        }
    }
    ...
    if (nodeRange !== undefined && !accepts(NODE_RUNTIME_VERSION, nodeRange)) {
        throw new MarketInstallError('verification-failed', 'The plugin package does not support the Node.js runtime bundled with DSH Star.');
    }
}
...
export function createNpmRegistryVerifier(http) {
    return {
        async verify(candidate, signal) {
            if (!safePackageName(candidate.packageName)
            ...
            if (finalOrigin !== NPM_REGISTRY_ORIGIN
                || metadata === null
...
async function readProfileLock(profile) {
    ...
    if (body.byteLength > MAX_LOCKFILE_BYTES)
...
function assertProfileLockRecord(lockfile, packageName, version, expectedIntegrity) {
    ...
    if (resolution?.integrity !== expectedIntegrity)
        throw new Error('lockfile integrity mismatch');
    const snapshot = lockEntry(record(lockfile.snapshots) ?? {}, [resolvedKey, `/${resolvedKey}`]);
    if (snapshot === undefined)
        throw new Error('lockfile snapshot missing');
}
...
async function assertInstalledBundleFromSnapshot(snapshot, packageName, version, expectedPatch, expectedIntegrity) {
    if (profileDependency(snapshot.manifest, packageName) !== version)
        throw new Error('dependency mismatch');
    if (!profileBundles(snapshot.manifest).includes(packageName))
    ...
    if (!containedPath(snapshot.resolvedNodeModules, resolvedPackageDir))
        throw new Error('package escaped profile');
    const manifest = await readManifest(join(resolvedPackageDir, 'package.json'));
    if (manifest.name !== packageName || manifest.version !== version)
        throw new Error('installed package mismatch');
...
async function assertInstalledBundle(profile, packageName, version, expectedPatch, expectedIntegrity) {
    await assertInstalledBundleFromSnapshot(await loadInstalledProfileSnapshot(profile), packageName, version, expectedPatch, expectedIntegrity);
}
...
export class MarketInstallService {
    ...
    constructor(scope, currentProfile, pnpm, verifier, options = {}) {
        ...
        for (const [label, value] of [
            ...
            if (!Number.isSafeInteger(value) || value < 1)
                throw new TypeError(`invalid market install ${label}`);
        }
    }
    observeCatalog(snapshot) {
        ...
        for (const item of snapshot.items) {
            ...
            if (item.provenance.sourceRecordId !== snapshot.source.sourceRecordId
    ...
    invalidateSource(sourceRecordId) {
        ...
        for (const [token, intent] of this.intents) {
            if (intent.kind === 'install' && intent.candidate.sourceRecordId === sourceRecordId)
    ...
    async listVerifiedReceipts(signal = this.generation.signal) {
        ...
        for (const receipt of receipts) {
            try {
                await assertInstalledBundleFromSnapshot(snapshot, receipt.packageName, receipt.version, receipt.bundlePatch, receipt.integrity);
                operationSignal.throwIfAborted();
    ...
    async listInstallable(index, signal) {
        ...
        operationSignal.throwIfAborted();
        this.purge();
        const currentKeys = new Set(index.snapshots.flatMap(snapshot => (snapshot.items.map(item => candidateKey(index.source.sourceRecordId, item.id)))));
        for (const [key, candidate] of this.candidates) {
    ...
    async executeInstall(token, signal) {
        return await this.runExclusive(async () => {
            ...
            try {
                await this.runPlugin(this.installOptions(candidate.packageName), profile, operationSignal, true, {
                ...
                if (!await this.installMayHaveMutatedProfile(profile, candidate.packageName))
                    throw cause;
                await this.rollbackInstall(profile, candidate.packageName, receipt.receiptId);
                throw new MarketInstallError('operation-failed', 'The package manager failed after changing the active profile, so the partial installation was rolled back.');
            }
            try {
                await assertInstalledBundle(profile, candidate.packageName, candidate.version, verification.bundlePatch, verification.integrity);
                operationSignal.throwIfAborted();
            }
            catch {
                await this.rollbackInstall(profile, candidate.packageName, receipt.receiptId);
                throw new MarketInstallError('operation-failed', 'The package manager finished, but the plugin bundle was invalid, so the installation was rolled back.');
            }
            try {
    ...
    async executePreview(token, signal) {
        ...
        const result = intent.kind === 'install'
    ...
    async executeUninstall(token, signal) {
        return await this.runExclusive(async () => {
            ...
            try {
                await assertInstalledBundle(profile, currentReceipt.packageName, currentReceipt.version, currentReceipt.bundlePatch, currentReceipt.integrity);
                operationSignal.throwIfAborted();
            }
            catch {
                throw new MarketInstallError('conflict', 'The installed plugin no longer matches its market receipt.');
    ...
    async saveReceipts(receipts) {
        if (receipts.length > MAX_RECEIPTS || !receipts.every(validReceipt))
    ...
    async ensureRecoveredInstallReconciled() {
        ...
        try {
            ...
            if (this.recoveryReconciliation === operation)
                this.recoveryReconciliation = undefined;
            throw cause;
        }
    }
    async reconcileRecoveredInstall() {
        ...
        if (retained.length !== current.length)
            await this.saveReceipts(retained);
        for (const receiptId of uniqueIds)
            await this.pnpm.acknowledgeRecoveredInstall(receiptId);
    }
    issueIntent(intent) {
        this.assertOpen();
        this.purge();
        let token = opaqueToken();
        while (this.intents.has(token))
            token = opaqueToken();
    ...
    issueRestartToken() {
        ...
        this.purge();
        let token = opaqueToken();
        while (this.restartIntents.has(token))
            token = opaqueToken();
    ...
    async runExclusive(task) {
        ...
        if (this.operationActive)
            throw new MarketInstallError('conflict', 'Another market package operation is already running.');
    ...
    assertOpen() {
        if (this.closed)
            throw new MarketInstallError('operation-failed', 'The market install service is unavailable.');
    }
    ...
    async rollbackInstall(profile, packageName, receiptId) {
        try {
            ...
            throw new MarketInstallError('persistence-failed', 'The failed installation could not be restored safely. Use the saved recovery state before another plugin change.');
        }
    }
}
