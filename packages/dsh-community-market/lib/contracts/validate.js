import { CatalogContractError, schemaIssues, semanticIssue } from './errors.js';
...
export function parseCatalogSource(value) {
    ...
    if (issues.length)
        throw new CatalogContractError('source', issues);
    return source;
}
export function parseCatalogQuery(value) {
    return parseSchema('query', validators.query, value);
}
export function parseCatalogProviderPage(value, effectiveLimit) {
    ...
    for (const [index, item] of page.items.entries()) {
        ...
        seen.add(item.id);
        if (item.repository)
            normalizeRepositoryIdentity(item.repository);
    }
    return page;
}
export function parseCatalogSnapshot(value) {
...
export function validateLocalSourceRecords(records) {
    const ids = new Set();
