...
import { CatalogNetworkError, createCachedCatalogHttpClient, createRestrictedHttpClient, restrictedHttpClient, } from '../network/restricted-http.js';
import { DSH_1024STORE_ADAPTER_ID, DSH_1024STORE_HOSTNAME, } from '../adapters/dsh-1024store.js';
...
import { SettingsCatalogSourceStore } from '../catalog/source-store.js';
...
const dsh1024StoreHttpClient = createCachedCatalogHttpClient(createRestrictedHttpClient({
    syntheticProxyHostnames: [DSH_1024STORE_HOSTNAME],
    maxBodyBytes: MAX_DSH_1024STORE_BODY_BYTES,
}));
const dshfindHttpClient = createCachedCatalogHttpClient(createRestrictedHttpClient({
...
function sendCatalogFailure(res, cause) {
    ...
    else if (cause instanceof CatalogNetworkError && cause.code === 'response'
...
function cachedCatalogResponse(cache, source, locale, now = Date.now()) {
    ...
    if (snapshot.source.sourceRecordId !== source.sourceRecordId
...
function catalogCacheFromResponse(response, sourceRecordId, locale, now = Date.now()) {
    ...
    if (snapshot === undefined || metadata === undefined
        ...
        || !response.categories.every(value => typeof value === 'string'))
        return undefined;
...
export function marketRequestAllowed(context) {
    return marketAuthority(context) !== undefined;
}
export function marketMutationAllowed(context) {
...
function asMutation(value) {
    ...
    if (mutation.action === 'add-standard' && typeof mutation.manifestUrl === 'string')
        return { action: 'add-standard', manifestUrl: mutation.manifestUrl };
    ...
    if (mutation.action === 'remove' && typeof mutation.sourceRecordId === 'string')
        return { action: 'remove', sourceRecordId: mutation.sourceRecordId };
...
function reconcileInstallations(receipts, value) {
    ...
    for (const bundle of value)
        packageCounts.set(bundle.packageName, (packageCounts.get(bundle.packageName) ?? 0) + 1);
...
export async function readStandardSourceManifest(manifestUrl, signal, http = restrictedHttpClient) {
...
async function mutateSources(scope, mutation, signal, onUnavailable, readManifest = readStandardSourceManifest) {
    signal.throwIfAborted();
    const store = new SettingsCatalogSourceStore(scope);
    const records = [...await store.load()];
    const unavailableSourceRecordIds = new Set();
    ...
    if (mutation.action === 'add-builtin') {
        ...
        if (provider === undefined)
            throw new Error('built-in source unavailable');
        if (records.some(record => record.builtInProviderKey === mutation.key))
        ...
        signal.throwIfAborted();
        if (records.some(record => record.manifestUrl === mutation.manifestUrl))
            throw new Error('source already added');
        records.push({
            sourceRecordId: randomUUID(),
        ...
        if (mutation.action === 'remove') {
            ...
            for (const [recordIndex, record] of records.entries()) {
                ...
                if (record.enabled && !enabled)
                    unavailableSourceRecordIds.add(record.sourceRecordId);
        ...
        if (targetIndex < 0 || targetIndex >= ordered.length)
            throw new Error('source cannot move further');
    ...
    for (const sourceRecordId of unavailableSourceRecordIds)
        onUnavailable?.(sourceRecordId);
}
export function createMarketSourceMutator(scope, onUnavailable, readManifest) {
...
export function registerMarketRoutes(ctx, scope, installProvider, desktopActionsProvider, desktopPluginsProvider) {
    const expectedPort = ctx.webServer.port;
    const generationController = new AbortController();
    const disablePreviews = new Map();
    const enablePreviews = new Map();
    const desktopPluginRestartTokens = new Map();
    const purgeDesktopTokens = () => {
        const now = Date.now();
        for (const [token, preview] of disablePreviews) {
            if (now >= preview.expiresAt)
                disablePreviews.delete(token);
        }
        for (const [token, preview] of enablePreviews) {
            if (now >= preview.expiresAt)
                enablePreviews.delete(token);
        }
        for (const [token, expiresAt] of desktopPluginRestartTokens) {
            if (now >= expiresAt)
                desktopPluginRestartTokens.delete(token);
        }
    };
    ...
    const servedCatalogPreviews = new Set();
    ...
    const mutateSource = createMarketSourceMutator(scope, sourceRecordId => {
        ...
        for (const key of servedCatalogPreviews) {
            if (key.startsWith(`${sourceRecordId}\0`))
                servedCatalogPreviews.delete(key);
        }
    ...
    const routes = [
        ctx.webServer.register({ kind: 'exact', path: ROUTE_STATE, handler: async (_req, res) => {
                ...
                try {
                    ...
                    if (!generationController.signal.aborted && !res.destroyed)
        ...
        ctx.webServer.register({ kind: 'exact', path: ROUTE_CATALOG, handler: async (req, res) => {
                ...
                try {
                    ...
                    const scope = sourceRecordIds.length === 0
                        ? undefined
                        : {
                    ...
                    const activeSource = scope === undefined
                        ? undefined
                        : (await service.listSources()).find(source => (source.enabled && source.sourceRecordId === scope.sourceRecordId));
                    if (scope !== undefined && activeSource === undefined)
                    ...
                    if (refreshPreviewKey !== undefined)
                        servedCatalogPreviews.delete(refreshPreviewKey);
                    if (!signal.aborted && !res.destroyed)
                        sendJson(res, 400, { error: 'invalid catalog query' });
                }
                finally {
                    stopWatching();
                }
        ...
        ctx.webServer.register({ kind: 'exact', path: ROUTE_SOURCES, handler: async (req, res) => {
                ...
                try {
                    ...
                    if (!signal.aborted && !res.destroyed)
                        sendJson(res, 200, { sources: await service.listSources() });
                }
                catch (cause) {
                    if (!signal.aborted && !res.destroyed) {
    ...
    if (desktopActionsProvider !== undefined) {
        routes.push(ctx.webServer.register({ kind: 'exact', path: ROUTE_OPEN_TERMINAL, handler: async (req, res) => {
                if (req.method !== 'POST' || !mutationAllowed(req, expectedPort)) {
    ...
    if (installProvider !== undefined) {
        routes.push(ctx.webServer.register({ kind: 'exact', path: ROUTE_INSTALLABLE, handler: async (req, res) => {
                ...
                try {
                    ...
                    if (!generationController.signal.aborted) {
                        ...
                        const preview = buildCatalogResponse(index, { limit: 50, ...(localeKey === '' ? {} : { locale: localeKey }) }, { sourceRecordId: index.source.sourceRecordId });
                ...
                try {
                    ...
                    if (request.action === 'disable' || request.action === 'enable') {
                        ...
                        if (!target.mutable || target.status !== expectedStatus) {
                            throw new MarketInstallError('conflict', request.action === 'disable'
                        ...
                        if (!boundedIdentifier(preview.previewId)
                            || !boundedIdentifier(preview.profileName)
                            || !boundedIdentifier(preview.packageName)
                            || !Number.isFinite(expiresAt)
                            || expiresAt <= Date.now())
                            throw new MarketInstallError('operation-failed', request.action === 'disable'
                        ...
                        if (request.action === 'disable') {
                            rememberDisablePreview(preview.previewId, expiresAt, request.bundleId, preview.packageName, managedReceipt?.receiptId);
                        }
                        else {
                            rememberEnablePreview(preview.previewId, expiresAt, request.bundleId, preview.packageName, managedReceipt?.receiptId);
                        }
                        if (!signal.aborted && !res.destroyed) {
                            sendJson(res, 200, {
                        ...
                        if (!signal.aborted && !res.destroyed)
                            sendJson(res, 200, { ...summary, previewId: intent });
                    }
                }
                catch (cause) {
                    if (!signal.aborted && !res.destroyed)
                ...
                try {
                    ...
                    if (disablePreview !== undefined || enablePreview !== undefined) {
                        ...
                        if (currentTarget === undefined
                            ...
                            throw new MarketInstallError('conflict', action === 'disable'
                        ...
                        if (!boundedIdentifier(changed.packageName) || changed.packageName !== desktopPreview.packageName) {
                            throw new MarketInstallError('operation-failed', action === 'disable'
...
export function registerMarketSettings(ctx) {
